...
import java.awt.MouseInfo; // pre pozíciu myši
...
public class Manazer {
    ...
    private static final long TICK_LENGTH = 100000000; // zmenené z 0.25 na 0.1
    
    private class ManazerKlaves extends KeyAdapter {
        public void keyPressed(KeyEvent event) {
            if (event.getKeyCode() == KeyEvent.VK_A) {   // spawne turretku na danom indexe po stlačení A             
                Manazer.this.posliSpravu("placeTurret", mouseCoordinatesX(), mouseCoordinatesY());
    ...
    /*
     * Vypočíta index pola, kde sa práve nachádza myš
     */
    public int mouseCoordinatesX() { 
        return ((int)MouseInfo.getPointerInfo().getLocation().getX() - 35) / 70;       
    }
    /*
     * Vypočíta index pola, kde sa práve nachádza myš
     */
    public int mouseCoordinatesY() {
        return ((int)MouseInfo.getPointerInfo().getLocation().getY() - 35) / 70;       
    }
    ...
    private class ManazerMysi extends MouseAdapter {
        public void mouseClicked(MouseEvent event) {            
            if (event.getButton() == MouseEvent.BUTTON1) { // skipuje kolo
                Manazer.this.posliSpravu("mapClick", event.getX(), event.getY());
            }            
            if (event.getButton() == MouseEvent.BUTTON3) { // rozbije turretu
                Manazer.this.posliSpravu("breakTurret", event.getX(), event.getY());
            }
        }
    }
